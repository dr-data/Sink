          ...
          try {
            if (key.metadata?.url) {
              ...
              if (link) {
                ...
                // Optimization: Removing inline KV.put (database migration) during read operation
                // to prevent excessive writes and performance degradation in serverless/KV environment.
                // The migration is instead handled by a separate Nitro task.
              }
            }
          }
          catch (err) {
